...
export class PywwsDataset {
  ...
  get(...params) {
    if (__WEBPACK__) {
      let [d1, d2, columns] = params;
      let url = `${this.type}/${d1}/${d2}`;
      return query(url, columns);
    }
    else {
  ...
  getOne(...params) {
    if (__WEBPACK__) {
      let [date, columns] = params;
      let url = `${this.type}/${date}`;
      return query(url, columns);
    }
    else {
